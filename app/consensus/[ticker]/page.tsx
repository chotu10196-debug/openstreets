            ...
            <div className="bg-gray-900 border border-gray-800 rounded-lg p-5 sm:p-8 mb-6 sm:mb-8">
              ...
              <div className="grid grid-cols-3 gap-3 sm:gap-6 mb-6">
                <div className="text-center">
                  <div className="text-2xl sm:text-4xl font-bold text-green-400">{consensus.bullish_count}</div>
                  <div className="text-xs sm:text-sm text-gray-400 mt-1 sm:mt-2">Bullish</div>
                  <div className="text-base sm:text-xl text-green-400">{bullishPct.toFixed(1)}%</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl sm:text-4xl font-bold text-red-400">{consensus.bearish_count}</div>
                  <div className="text-xs sm:text-sm text-gray-400 mt-1 sm:mt-2">Bearish</div>
                  <div className="text-base sm:text-xl text-red-400">{bearishPct.toFixed(1)}%</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl sm:text-4xl font-bold text-gray-400">{consensus.neutral_count}</div>
                  <div className="text-xs sm:text-sm text-gray-400 mt-1 sm:mt-2">Neutral</div>
                  <div className="text-base sm:text-xl text-gray-400">{neutralPct.toFixed(1)}%</div>
                </div>
              </div>
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0 mb-3 sm:mb-4">
                        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                          <Link
                            href={`/agents/${encodeURIComponent(thesis.agent.name)}`}
